...
def main():
    ...
    if _pl.Path(cache).exists():
        ...
        json.dump({"P": [v.tolist() for v in P],
                   # Подпись обязана описывать ТУ композицию, которой посчитано. Здесь стояло
                   # "SOLO на CYP3A4" --- верно до пунктов 282-285, после них неверно, и если
                   # кэш когда-нибудь удалят, он пересоздался бы с ложной надписью. Берём
                   # состав из самого submit, чтобы подпись не могла разойтись с кодом.
                   "что": ("oof_predictions, ансамбль5, мёртвая зона, SOLO "
                           + "; ".join(f"{k}={'+'.join(v)}" for k, v in SB.SOLO.items())
                           + ", сид 0")},
                  open(cache, "w"))
