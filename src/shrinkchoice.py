...
DELTAS = np.round(np.arange(-0.4, 0.81, 0.1), 1)   # отрицательные нужны для 2D6
...
def main():
  ...
  а 2D6 --- единственный фермент, у которого основания активнее. Разбор: verify/k7_2d6shift.py.""")
    ...
    print("=" * 94)
    print("""   Диапазон истины строится из измеренного: оценка k5 --- один край (она нижняя
   граница по модулю, потому что модель проводит в свои выходы лишь часть входного
   сдвига), запас в сторону её знака --- другой. У CYP2D6 вторым краем служит ноль:
   якорный контроль говорит, что обогащения там нет, а k5 --- что сдвиг отрицателен.
   ...
   выбор правила под оценку и проверка по ней же были бы одним и тем же действием.\n""")
    # Диапазоны построены из измеренного, а не из рассуждения о конструкции набора.
    # Основа --- КЛАСТЕРНЫЙ бутстрап сдвига предсказаний: тест состоит из якорей с
    # аналогами, поэтому бутстрап по молекулам занижает ошибку, и измеренный design effect
    # по дисперсии равен 4.4-6.7, то есть ошибка занижена вдвое-втрое. Кластерные интервалы:
    # 1A2 [-0.111,+0.124], 2C9 [+0.025,+0.257], 2D6 [-0.281,-0.093], 3A4 [+0.295,+0.576].
    # К ним добавлено 0.05 на неопределённость обращения: доля входного сдвига, доходящая
    # до выхода, известна не точно, а сам множитель зависит от того, вдоль какой оси идёт
    # сдвиг (verify/k7_2d6shift.py и обсуждение в разделе о решающем слое).
    TRUTH = {"CYP1A2": (-0.2, 0.2), "CYP2C9": (0.0, 0.3),
             "CYP2D6": (-0.3, -0.1), "CYP3A4": (0.3, 0.6)}
    AC = {c: np.mean([MC[s][c] for s in SEEDS], axis=0) for c in CYPS}
    K5 = {"CYP1A2": 0.018, "CYP2C9": 0.164, "CYP2D6": -0.256, "CYP3A4": 0.437}
