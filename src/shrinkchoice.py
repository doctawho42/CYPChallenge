...
OFFGRID = np.round(np.arange(-3.0, 3.01, 0.05), 2)   # шире, чем нужно: край сетки лжёт
...
def main():
  ...
  а 2D6 --- единственный фермент, у которого основания активнее. Разбор: verify/k7_2d6shift.py.""")
   ...
   несёт ровно тот дефект, за который отвергнуто единое правило.""")
        ...
        print()
        print("=" * 94)
        print("8. Излом: насколько может ошибаться оценка, прежде чем ячейка станет хуже нуля")
        print("=" * 94)
        print("""   Вероятность отвечает на вопрос о выборочном шуме. Систематическую ошибку ---
   а именно ею грозит невыполнение инвариантности ядра --- апостериорное распределение
   не содержит вовсе. Правильный вопрос к правилу поэтому не «какова вероятность», а
   «на сколько может съехать истинное delta, прежде чем правило проиграет бездействию».\n""")
        print(f"{'фермент':8s} {'выбрано':>8s} {'E[delta]':>9s} {'излом':>8s} {'запас':>8s}")
        fine = np.round(np.arange(DELTAS[0], DELTAS[-1] + 1e-9, 0.01), 2)
        for c in CYPS:
            ib_ = int(np.where(np.isclose(DELTAS, picks[c][1]))[0][0])
            if picks[c][1] == 0.0:
                print(f"{c:8s} {0.0:+8.1f} {float(DR[c].mean()):9.3f} {'--':>8s} "
                      f"{'ноль, нечему ломаться':>8s}")
                continue
            vr = np.interp(fine, DELTAS, AC[c][ib_, :])
            vz = np.interp(fine, DELTAS, AC[c][i0, :])
            d = vr - vz
            em = float(DR[c].mean())
            # ближайший к E[delta] переход через ноль разности
            sgn = np.sign(d)
            cross = [fine[k] for k in range(1, len(fine)) if sgn[k] != sgn[k - 1]]
            near = min(cross, key=lambda x: abs(x - em)) if cross else np.nan
            print(f"{c:8s} {picks[c][1]:+8.1f} {em:9.3f} {near:+8.2f} "
                  f"{abs(em - near):8.2f}")
        print("""
   Запас читать против уже применённых поправок. Стратификация ядра подвинула CYP2D6 на
   0.41 --- если запас там меньше этого, значит одна уже сделанная поправка была больше
   того, что осталось, и вторая ось состава сопоставимого веса перевернула бы ячейку.""")
