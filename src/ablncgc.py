...
def main():
    ...
    for seed in seeds:
        ...
        for arm in arms:
            ...
            # Разбор веса из имени руки. Первая версия делала split("w")[1].split()[0] и на
            # руке "...w0.1, перемешанный" получала "0.1," с запятой --- три прогона подряд
            # падали на float(). Регулярное выражение вместо разрезания по букве.
            _mw = _re.search(r"\bw([0-9.]+)", arm)
            w_pan = float(_mw.group(1)) if _mw else 1.0
